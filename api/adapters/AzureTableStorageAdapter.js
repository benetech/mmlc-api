...
/**
 * Sails Boilerplate Adapter -- DEMO for Azure Table Storage.
 * If we decide to use table storage, we should finish building this out.
 * To use this, make sure to fill out the info in /config.json with your
 * Azure Table Storage account information.
 *
  ...
  var adapter = {
    ...
    find: function(collectionName, options, cb) {
	  ...
	  var query = azure.TableQuery
	    ...
	    .and('RowKey eq ? ', options.where.RowKey);
	  
	  tableService.queryEntities(collectionName, partitionKey, options.where.RowKey, function(error, entity) {
		console.log(error);
	  	if (!error) {
			cb(null, [entity]);
		} else {
    ...
    create: function(collectionName, values, cb) {
      values.PartitionKey = partitionKey;
	  values.RowKey = uuid();
	  tableService.createTableIfNotExists(collectionName, function(error){
	    if(!error){
	        tableService.insertEntity(collectionName, values, function(err){
    ...
    */
	identity: 'sails-azure-table-storage'
